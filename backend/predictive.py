...
def _normalize(telemetry: Dict[str, Any]) -> Dict[str, float]:
    return {
        "temperature": max(0, (telemetry.get("temperature", 0) - 85) / 25),
        "vibration": min(1, telemetry.get("vibration", 0) / 6),
        "oil_quality_contaminants_V_oil": max(0, 1 - telemetry.get("oil_quality_contaminants_V_oil", 1)),
        "vibration_rms_A_rms": min(1, telemetry.get("vibration_rms_A_rms", 0) / 8),
        "brake_pad_wear_percent": min(1, telemetry.get("brake_pad_wear_percent", 0) / 100),
        "battery_soh_percent": max(0, 1 - telemetry.get("battery_soh_percent", 100) / 100),
        "transmission_fluid_temp_C": max(0, (telemetry.get("transmission_fluid_temp_C", 0) - 80) / 60),
        "fuel_pressure_kPa": max(0, (350 - telemetry.get("fuel_pressure_kPa", 350)) / 250),
        "ev_battery_temp_C": max(0, (telemetry.get("ev_battery_temp_C", 0) - 40) / 35),
        "ev_voltage_stability": max(0, 1 - telemetry.get("ev_voltage_stability", 1)),
        "petrol_knock_index": min(1, telemetry.get("petrol_knock_index", 0) / 1.0),
        "petrol_fuel_trim": min(1, abs(telemetry.get("petrol_fuel_trim", 0)) / 25),
        "truck_axle_load_imbalance": min(1, telemetry.get("truck_axle_load_imbalance", 0)),
        "truck_brake_air_pressure": max(0, (90 - telemetry.get("truck_brake_air_pressure", 90)) / 40),
        "ambulance_high_rpm_flag": 1.0 if telemetry.get("ambulance_high_rpm_flag") else 0.0,
        "motorcycle_vibration": min(1, telemetry.get("motorcycle_vibration", 0) / 6),
        "motorcycle_lean_angle_deg": min(1, telemetry.get("motorcycle_lean_angle_deg", 0) / 60),
        "motorcycle_regulator_temp_C": max(0, (telemetry.get("motorcycle_regulator_temp_C", 0) - 70) / 50),
        "motorcycle_methane_ppm": min(1, telemetry.get("motorcycle_methane_ppm", 0) / 50),
        "petrol_air_fuel_ratio": min(1, abs(14.7 - telemetry.get("petrol_air_fuel_ratio", 14.7)) / 10),
        "petrol_injector_duty_cycle": min(1, telemetry.get("petrol_injector_duty_cycle", 0) / 100),
        "petrol_cranking_latency_ms": min(1, telemetry.get("petrol_cranking_latency_ms", 0) / 800),
        "petrol_delta_fuel_pressure_kPa": min(1, abs(telemetry.get("petrol_delta_fuel_pressure_kPa", 0)) / 80),
        "truck_exhaust_temp_C": max(0, (telemetry.get("truck_exhaust_temp_C", 0) - 450) / 400),
        "truck_thermal_variance": min(1, telemetry.get("truck_thermal_variance", 0) / 1.0),
        "truck_turbo_boost_kPa": max(0, (telemetry.get("truck_turbo_boost_kPa", 0) - 180) / 80),
        "ambulance_suspension_load": min(1, telemetry.get("ambulance_suspension_load", 0) / 1.0),
        "ambulance_cabin_co2_ppm": max(0, (telemetry.get("ambulance_cabin_co2_ppm", 400) - 800) / 2000),
        "ambulance_o2_tank_percent": max(0, (50 - telemetry.get("ambulance_o2_tank_percent", 100)) / 50),
        "ambulance_fridge_temp_C": max(0, (telemetry.get("ambulance_fridge_temp_C", 0) - 8) / 12),
        "ambulance_suction_pressure_kPa": max(0, (70 - telemetry.get("ambulance_suction_pressure_kPa", 70)) / 40),
        "ambulance_iv_flow_rate_ml_min": max(0, (10 - telemetry.get("ambulance_iv_flow_rate_ml_min", 40)) / 40),
        "ev_igbt_temp_C": max(0, (telemetry.get("ev_igbt_temp_C", 0) - 80) / 70),
        "ev_stator_temp_C": max(0, (telemetry.get("ev_stator_temp_C", 0) - 90) / 70),
        "ev_rotor_alignment_error": min(1, telemetry.get("ev_rotor_alignment_error", 0) / 0.5),
        "ev_bearing_vibration": min(1, telemetry.get("ev_bearing_vibration", 0) / 6),
        "ev_cell_delta_V": min(1, telemetry.get("ev_cell_delta_V", 0) / 0.2),
        "ev_internal_resistance_mOhm": max(0, (telemetry.get("ev_internal_resistance_mOhm", 2) - 6) / 20),
        "ev_contactor_temp_C": max(0, (telemetry.get("ev_contactor_temp_C", 0) - 70) / 50),
    }
...
def predict_breakdown_risk(telemetry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dominant_sensor, dominant_value = max(norm.items(), key=lambda kv: kv[1])
    ...
    return {
        ...
        "current_sensor_value": telemetry.get(dominant_sensor),
    }
