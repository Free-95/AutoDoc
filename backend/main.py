...
async def api_add_stock(req: AddStockRequest):
    success = add_stock(req.dealer_id, req.vehicle_id, req.model)
    if not success:
...
async def api_assign(req: AssignRequest):
    success, msg = assign_vehicle(req.dealer_id, req.vehicle_id, req.target_username)
    if not success:
...
async def vehicle_health(vehicle_id: str):
    history = VEHICLE_HEALTH_HISTORY.get(vehicle_id, [])
    if not history:
...
@app.get("/vehicle/{vehicle_id}/health/trend")
async def vehicle_health_trend(vehicle_id: str):
    history = VEHICLE_HEALTH_HISTORY.get(vehicle_id, [])
    if not history:
        raise HTTPException(404, "No telemetry yet for vehicle")
...
async def chatbot_query(payload: ChatbotQuery):
    ...
    alert = alert_service.get_alert_for_vehicle(vehicle_id)
    ...
    UEBA_CACHE[vehicle_id] = ueba_output
...
@app.get("/security/ueba/{vehicle_id}")
async def security_ueba(vehicle_id: str):
    return {"vehicle_id": vehicle_id, "ueba": UEBA_CACHE.get(vehicle_id, {"ueba_status": "NORMAL", "ueba_score": 0, "ueba_findings": ["No data"]})}