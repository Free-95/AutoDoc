...
def _serialize_dealer(dealer: Dealer) -> Dict:
    inventory = [
        {"id": v.vehicle_id, "model": v.model, "status": "Available"}
...
def add_stock(dealer_username: str, vehicle_id: str, model: str) -> bool:
    ...
    with session_scope() as session:
        ...
        if not dealer:
            return False
        existing = (
            session.query(Vehicle).filter(Vehicle.vehicle_id == vehicle_id).one_or_none()
        )
        if existing:
            return False
...
def assign_vehicle(
    dealer_username: str, vehicle_id: str, target_username: str
) -> Tuple[bool, str]:
    with session_scope() as session:
        dealer = (
            session.query(Dealer)
        ...
        vehicle = (
            ...
            .filter(Vehicle.vehicle_id == vehicle_id, Vehicle.dealer_id == dealer.dealer_id)
            .one_or_none()
        )
        if not vehicle:
