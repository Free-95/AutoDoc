...
def _ensure_vehicle(
    ...
    vehicle = session.query(Vehicle).filter(Vehicle.vehicle_id == vehicle_id).one_or_none()
    if vehicle:
        return vehicle
    sale_date = (
...
def ensure_seed_data():
    with session_scope() as session:
        tata_user = _ensure_user(
            session,
            "DLR_TATA",
            "Downtown Tata Motors",
            "ADMIN",
            "dealer@autodoc.local",
            "admin",
            "+91-022-99887766",
        )
        mahindra_user = _ensure_user(
            session,
            "DLR_MAHINDRA",
            "Highway Mahindra Hub",
            "ADMIN",
            "dealer2@autodoc.local",
            "admin",
            "+91-020-11223344",
        )
        rahul = _ensure_user(
        ...
        tata_dealer = _ensure_dealer(session, "DLR_TATA", "Mumbai, MH", "+91-022-99887766")
        mahindra_dealer = _ensure_dealer(
            session, "DLR_MAHINDRA", "Pune, MH", "+91-020-11223344"
        )
