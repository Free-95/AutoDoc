...
function App() {
  ...
  const addStock = async () => {
    ...
    try {
      const res = await axios.post("http://127.0.0.1:8000/dealer/add-stock", {
  ...
  const assignCar = async (vin) => {
    try {
      const res = await axios.post("http://127.0.0.1:8000/dealer/assign", {
  ...
  const toggleAttack = async () => {
    ...
    setAttackMode(next)
    await axios.post(`http://127.0.0.1:8000/toggle-attack/${next}`)
  }
  ...
  const bookService = async () => {
    ...
    try {
      const res = await axios.post("http://127.0.0.1:8000/book-service", {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`ws://127.0.0.1:8000/ws/1?vehicle_id=${selectedCar.id}&role=user`)
  ...
  useEffect(() => {
    const fetchSecurity = async () => {
      ...
      try {
        const res = await axios.get("http://127.0.0.1:8000/security/logs")
